...
for(i in 1:n.impute){
  imputed[[i]] <- impc[impc$.imp==i, c(1:7, 13:15)]
  imputed[[i]]$time=exp(imputed[[i]]$logtime)
  imputed[[i]]$status=survdat$status
  }
...
for (i in 1:N.clust){
  ...
  impc <- complete(imp.surv, action="long")
  for(j in 1:n.impute){
    imputed[[j]] <- impc[impc$.imp==j, c(1:7, 13:15)]
    imputed[[j]]$time=exp(imputed[[j]]$logtime)
    imputed[[j]]$status=data.in[[i]]$status
  }
